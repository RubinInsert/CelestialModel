...
class CelestialModel {
  ...
  static #orbitalShaderCode = {
    // Caching the shader codes on first load reduces network requests and speeds up rendering - TO BE REPLACED WITH A SINGLE SHADER POSSIBLY
    S: null,
    Px: null,
    Py: null,
    Pz: null,
  ...
  constructor(chemSymbol = "H", particleRatio = 1024) {
    this.chemSymbol =
      chemSymbol.charAt(0).toUpperCase() + chemSymbol.slice(1).toLowerCase();
    if (!CelestialModel.isInitialized) {
      throw new Error(
        "CelestialModel.init() must be called before creating instances."
      );
    }
    ...
    this.electronConfig =
      CelestialModel.#elementData[this.chemSymbol].electronConfig;
    this.protonFieldRadius =
      CelestialModel.#elementData[this.chemSymbol].protonFieldRadius ?? null;
    this.particleRatio = particleRatio;
    this.orbitals = [];
    this.highestOrbitalLevel = CelestialModel.#getHighestOrbitalLevel(
      this.electronConfig
    );
    this.#scaleFactor =
      CelestialModel.#elementData[this.chemSymbol].radius /
      Math.pow(this.highestOrbitalLevel, 2); // Used to scale the shader positions to the correct size internally
    this.maxDistance =
      Math.pow(this.highestOrbitalLevel, 2) * this.#scaleFactor;
  ...
  static updateParticles() {
    if (!CelestialModel.isInitialized) {
      throw new Error(
        "CelestialModel.init() must be called before using this method."
      );
    }
  ...
  update() {
    ...
    for (const shaderData of this.computeShaders) {
      ...
      shaderData.material.uniforms.texturePosition.value =
        shaderData.computeShader.getCurrentRenderTarget(
          shaderData.positionVariable
        ).texture;
      shaderData.positionVariable.material.uniforms.elapsedTime.value =
        elapsedTime;
      shaderData.positionVariable.material.uniforms.timeStep = {
        value: CelestialModel.timeStep,
      };
  ...
  static async #loadShader(url) {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to load shader from ${new URL(url, import.meta.url).href}: ${
          response.statusText
        }`
      );
    }
  ...
  static async #loadOrbitalShader(orbitalType) {
    if (CelestialModel.#orbitalShaderCode[orbitalType] !== null)
      return CelestialModel.#orbitalShaderCode[orbitalType];
    const response = await fetch(
      new URL(`./shaders/${orbitalType}/positionShader.glsl`, import.meta.url)
        .href
    );
    if (!response.ok) {
      throw new Error(
  ...
  static async #loadJSON(url) {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to load JSON from ${new URL(url, import.meta.url).href}: ${
          response.statusText
        }`
      );
    }
  ...
  static #fillTextures(positionTexture, orbitalLevel = 1) {
    ...
    for (let i = 0; i < posArray.length; i += 4) {
      // Random positions
      let point = CelestialModel.#generateRandomPointInRange(
        -orbitalLevel / 2,
        orbitalLevel / 2
      );
  ...
  async #createOrbital(
    numParticles = 4096,
    orbitalType = CelestialModel.OrbitalType.S,
    orbitalLevel = 1
  ) {
    if (!CelestialModel.isInitialized) return false;
    const positionShaderCode = await CelestialModel.#loadOrbitalShader(
      orbitalType
    );
    ...
    positionVariable.material.uniforms.timeStep = {
      value: CelestialModel.timeStep,
    }; // Initialize deltaTime
    positionVariable.material.uniforms.resolution = {
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: {
        atomicEmissionSpectrum: {
          value: CelestialModel.#createSpectrumTexture(
            CelestialModel.#elementData[this.chemSymbol].emissionSpectra
          ),
        },
        texturePosition: { value: null },
        scale: {
          value: (Math.pow(orbitalLevel, 2) * this.#scaleFactor) / 41.0,
        }, // Dividing by 41.0 is necessary because the orbital 1s has a max distribution of 41 units - Simply due to how I created the shader (So we scale down to a unit sphere)
  ...
  #createProtonField() {
    if (!this.protonFieldRadius) {
      console.warn(
        `No proton field radius defined for ${this.chemSymbol}. Proton field will not be created.`
      );
    }
    const protonFieldGeometry = new THREE.SphereGeometry(
      this.protonFieldRadius,
      64,
      64
    );
      ...
      uniforms: {
        ...
        uColor: {
          value: new THREE.Color(
            CelestialModel.#elementData[this.chemSymbol].emissionSpectra.at(-1)
          ),
    ...
    thicknessMaterial.onBeforeRender = (renderer, scene, camera) => {
      if (thicknessMaterial.userData.shader) {
        thicknessMaterial.userData.shader.uniforms.uCameraPosition.value.copy(
          camera.position
        );
      }
    };
    const protonFieldMaterial = new THREE.MeshBasicMaterial({
      color:
        CelestialModel.#elementData[this.chemSymbol].emissionSpectra.at(-1), // Default to red if no color is defined
      transparent: true,
      opacity: 0.5,
      depthWrite: false,
    });
  ...
  static #getEmissionSpectrum(elementName) {
    // This is for DEBUGGING ONLY - REMOVE LATER
    const elementWaveLengths =
      CelestialModel.#elementData[elementName].emissionSpectra;
    if (!elementWaveLengths) {
      console.error(
        `No emission spectrum data found for element: ${elementName}`
      );
      return;
    }
    let spectrumTexture =
      CelestialModel.#createSpectrumTexture(elementWaveLengths);
  ...
  async #createFromElectronConfig() {
    ...
    lastTwoShells.forEach((orbit) => {
      const orbitalLevel = parseInt(orbit[0]);
      this.highestOrbitalLevel = Math.max(
        this.highestOrbitalLevel,
        orbitalLevel
      );
      ...
      switch (orbitalType) {
        case "S":
          orbitalPromises.push(
            this.#createOrbital(
              this.particleRatio * orbitalElectronCount,
              "S",
              orbitalLevel
            )
          );
          break;
        case "P":
          particlesPerSuborbital = Math.max(
            1,
            Math.floor((this.particleRatio * orbitalElectronCount) / 3)
          );
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Px", orbitalLevel)
          );
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Py", orbitalLevel)
          );
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Pz", orbitalLevel)
          );
          break;
        case "D":
          particlesPerSuborbital = Math.max(
            1,
            Math.floor((this.particleRatio * orbitalElectronCount) / 5)
          );
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Dxy", orbitalLevel)
          );
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Dxz", orbitalLevel)
          );
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Dyz", orbitalLevel)
          );
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Dx2y2", orbitalLevel)
          );
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Dz2", orbitalLevel)
          );
          break;
        case "F":
          particlesPerSuborbital = Math.max(
            1,
            Math.floor((this.particleRatio * orbitalElectronCount) / 7)
          );
          orbitalPromises.push(
            this.#createOrbital(
              particlesPerSuborbital,
              "Fx_x2_minus_3y2",
              orbitalLevel
            )
          );
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Fxyz", orbitalLevel)
          );
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Fxz2", orbitalLevel)
          );
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Fy3", orbitalLevel)
          );
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Fyz2", orbitalLevel)
          );
          orbitalPromises.push(
            this.#createOrbital(
              particlesPerSuborbital,
          ...
          orbitalPromises.push(
            this.#createOrbital(particlesPerSuborbital, "Fz3", orbitalLevel)
          );
          break;
        default:
  ...
  async create() {
    ...
    return await this.#createFromElectronConfig(
      this.electronConfig,
      this.sqrtElectronRatio
    );
  }
  remove() {
  ...
  getTopTwoOrbitals() {
    return this.orbitals.filter(
      (orbit) => orbit.orbitalLevel >= this.highestOrbitalLevel - 2
    );
  }
  setProtonFieldVisibility(visible) {
    if (!this.protonField) {
      console.warn(
        `No proton field defined for ${this.chemSymbol}. Cannot set visibility.`
      );
      return;
    }
    ...
    unpairedElectronsInfo.unpairedOrbitals.forEach((orbit) => {
      let spinStateColor =
        Math.random() < 0.5
          ...
          : new THREE.Color(0xff0000); // Randomly choose blue or red
      ...
      if (isEnabled) {
        orbitalObjects.forEach((orbitObject) => {
          if (orbitObject.particles && orbitObject.particles.material) {
            ...
            orbitObject.particles.material.fragmentShader =
              CelestialModel.#spinStateFragShaderCode;
            orbitObject.particles.material.uniforms.spinStateColor = {
              value: spinStateColor,
            };
            orbitObject.particles.material.uniforms.spinStateRatio = {
              value: orbit.probabilityUnpaired,
            };
        ...
        this.orbitals.forEach((orbit) => {
          if (orbit.particles && orbit.particles.material) {
            orbit.particles.material.uniforms.spinStateFadeFactor = {
              value: CelestialModel.spinStateFadeFactor,
            }; // Set all paired particles to a faded/muted color.
        ...
        this.orbitals.forEach((orbit) => {
          if (orbit.particles && orbit.particles.material) {
            orbit.particles.material.uniforms.spinStateFadeFactor = {
              value: 1.0,
            }; // Set the faded particles back to normal color.
            orbit.particles.material.needsUpdate = true;
          }
        });
      }
    ...
    return this.orbitals.filter(
      (orbit) =>
        ...
        (orbit.orbitalType === orbitalType ||
          ...
          (orbitalType === "P" &&
            ["Px", "Py", "Pz"].includes(orbit.orbitalType)) ||
          (orbitalType === "D" &&
            ["Dxy", "Dxz", "Dyz", "Dx2y2", "Dz2"].includes(
              orbit.orbitalType
            )) ||
  ...
  getUnpairedElectrons() {
    ...
    const orbitals = this.electronConfig.match(/\d+[spdf]\d+/g).map((str) => {
